...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    classes_path    = "classes/license_plate.txt"
    #---------------------------------------------------------------------#
    #   anchors_path    代表先验框对应的txt文件，一般不修改。
    #   anchors_mask    用于帮助代码找到对应的先验框，一般不修改。
    #---------------------------------------------------------------------#
    anchors_path    = "anchors/license_plate.txt"
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    teacher_backbone     = "cspdarknet53"
    student_backbone     = "repvit_m0_6" # repvit_m0_6 / mobilenetv2_05 / cspdarknet53
    str_pretrained_path  = ""
    pretrained_path      = ""
    resume               = False
    checkpoint_path      = ""
    ...
    #------------------------------------------------------#
    # Teacher Model
    print("Set up teacher model...")
    teacher_model = YoloBody(teacher_backbone, anchors_mask, num_classes, pretrained_path = pretrained_path)
    if not pretrained_path:
        weights_init(teacher_model)
    if teacher_model_path != '':
        #------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        #------------------------------------------------------#
        if local_rank == 0:
            print('Load weights {}.'.format(teacher_model_path))
    ...
    student_model = YoloBody(student_backbone, anchors_mask, num_classes, pretrained_path = pretrained_path)
    if not pretrained_path:
    ...
    if student_model_path != '':
        ...
        #------------------------------------------------------#
        if local_rank == 0:
            print('Load weights {}.'.format(student_model_path))
        
        #------------------------------------------------------#
        #   根据预训练权重的Key和模型的Key进行加载
        #------------------------------------------------------#
        model_dict      = student_model.backbone.model_without_connector.state_dict()
        pretrained_dict = torch.load(student_model_path, map_location = device)
        pretrained_dict = pretrained_dict["model"]
        load_key, no_load_key, temp_dict = [], [], {}
        for k, v in pretrained_dict.items():
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                ...
                no_load_key.append(k)
        model_dict.update(temp_dict)
        student_model.backbone.model_without_connector.load_state_dict(model_dict)
        #------------------------------------------------------#
        #   显示没有匹配上的Key
        #------------------------------------------------------#
        if local_rank == 0:
    ...
    if local_rank == 0:
        ...
        log_dir         = os.path.join(save_dir, "CrossKD_" + student_backbone + "_" + dataset_name + "_" + "seed" + str(seed) + log_pretrained_path)
    ...
    if local_rank == 0:
        show_config(
            classes_path = classes_path, anchors_path = anchors_path, anchors_mask = anchors_mask, model_path = student_model_path, input_shape = input_shape, \
    ...
    if True:
        ...
        #------------------------------------#
        if Freeze_Train:
            for param in student_model.backbone.parameters():
        ...
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        for k, v in student_model.named_modules():
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(student_model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines, log_dir, Cuda, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(student_model_train, student_model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, log_dir, local_rank)
