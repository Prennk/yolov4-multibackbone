...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, T,
        ...
        print(f"MobileNetV2 {T}, {width_mult}")
...
class AdditionalConnectorToNeck(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(AdditionalConnectorToNeck, self).__init__()
...
class MobileNetV2WithConnectorToNeck(nn.Module):
    def __init__(self):
        super(MobileNetV2WithConnectorToNeck, self).__init__()
        self.mobilenetv2 = mobile_half(100)
        self.downsample_f2 = AdditionalConnectorToNeck(in_channels=16, out_channels=256)
        self.downsample_f3 = AdditionalConnectorToNeck(in_channels=48, out_channels=512)
        self.downsample_f4 = AdditionalConnectorToNeck(in_channels=160, out_channels=1024)

    def forward(self, x):
        feats = self.mobilenetv2(x, is_feat=True, preact=False)

        f2_downsampled = self.downsample_f2(feats[0])  # f2: [2, 16, 104, 104] -> [2, 256, 52, 52]
        f3_downsampled = self.downsample_f3(feats[1])  # f3: [2, 48, 52, 52] -> [2, 512, 26, 26]
        f4_downsampled = self.downsample_f4(feats[2])  # f4: [2, 160, 26, 26] -> [2, 1024, 13, 13]

        return f2_downsampled, f3_downsampled, f4_downsampled

def mobile_with_connector(pretrained):
    if pretrained:
