...
class YOLOLoss(nn.Module):
    ...
    def compute_crosskd_loss(self, cross_out, teacher_out):
        def quality_focal_loss(pred, target, gamma=2.0):
            ...
            diff = torch.abs(pred_sigmoid - target_sigmoid)
            modulating_factor = diff ** gamma
        ...
        def kl_divergence(pred, target):
            ...
            return F.kl_div(pred_log_softmax, target_softmax, reduction='batchmean')
        ...
        for cross, teacher in zip(cross_out, teacher_out):
            ...
            # Regresi (KL Divergence)
            reg_loss += kl_divergence(cross[..., :4], teacher[..., :4])
